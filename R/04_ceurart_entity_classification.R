...
# | `device = cairo_pdf` for Analysis 4bd_v (deviates from the project-wide `device = "pdf"` convention in SKILL.md §6) | The poster (CEUR-WS `ceurart` class) typesets all text, including figure captions, in Libertinus Sans; base R's `pdf()` device can only embed the 14 base PDF fonts (Helvetica, Times, …) and silently substitutes Helvetica for any other requested family. `cairo_pdf` renders through Cairo/fontconfig and can embed arbitrary installed fonts, so `theme_minimal(base_family = "Libertinus Sans")` is actually honoured in the exported PDF. |
#
# INTERPRETATION
# --------------
...
p4b_vert <- ggplot(df4b, aes(x = source, y = pct, fill = entity_type)) +
  ...
  theme_minimal(base_size = 13, base_family = "Libertinus Sans") +
  theme(
...
# Own addition: cairo_pdf instead of the project-wide `device = "pdf"`
# convention (SKILL.md §6) — required to embed Libertinus Sans (base R's
# pdf() device silently substitutes Helvetica for non-base-14 font families).
